        ...
        <div className="mt-10 space-y-7">
          ...
          <Field label="예약 날짜" required>
            ...
            {date && (
              <div className="mt-4">
                {loadingTimes ? (
                  <p className="text-xs text-muted">실시간 예약 현황 불러오는 중…</p>
                ) : slots.length === 0 ? (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !selected && available && "border-line bg-white text-ink hover:bg-neutral-50",
        !available &&
          "cursor-not-allowed border-line bg-neutral-50 text-muted/60",
      )}
    >
      <span className={cn("tabular-nums", !available && "line-through")}>
        {time}
      </span>
      {!available && <span className="text-[10px]">불가</span>}
    </button>
  );
}
...
/**
 * Build the 30-min slot grid for a given date. A slot is unavailable when
 * its window [start, start+duration) overflows shop hours, falls in the past
 * (today only), overlaps the break window, or collides with any pending /
 * confirmed reservation.
 *
 * Returning `available: false` slots (rather than filtering) is intentional —
 * the customer-facing form shows the unavailable times as "불가" pills so the
 * shop's actual schedule is visible.
 */
